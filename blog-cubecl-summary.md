# CubeCL 不是语言，是编译器框架：`#[cube]` 如何把 Rust 函数变成六平台的 GPU 代码

## 读前须知

- **CubeCL 是什么**：Tracel 开源的 Rust **GPU 计算库 + 编译器框架**——不是一门新语言。你用 `#[cube]` 和 CubeCL 前端类型描述 kernel，在**首次 launch** 时 JIT 到 CUDA、ROCm/HIP、WebGPU/Metal/Vulkan、本机 CPU SIMD 等路径，而不是为每个平台各写一份 `.cu` / `.wgsl` / `.metal` 并做 FFI。
...
- **两种读法**：① **纵向**——作为 Burn 底层机制系列的 GPU 篇，接在 [Burn 综合地图（类型栈 + 融合流）](blog-burn-summary.md) 和 [ONNX AOT 编译器](blog-burn-onnx-summary.md) 之后；② **横向**——**只关心 CubeCL、不读 Burn** 时，把本文当**机制地图**查阅即可，**跟练路径**见 **[CubeCL 专题计划](blog-cubecl-plan.md)**（建议从 **[第一章 · GELU launch](blog-cubecl-1.md)** 跑通示例）。
...
> `#[cube]` **过程宏（proc-macro）** 保留你的 Rust 函数供类型检查，并生成以函数名命名的子模块——其中的 **`expand`** 在首次 launch、经 runtime **`define()`** 调用时，向 **IR** 的 **`Scope`** 里填入 **`Operation`** 指令。你写的源码**不是**直接变成 GPU 机器码——你写的是一段在 **JIT** 时运行的 **IR 生成器**；再由 `cubecl-opt` 与各后端（如 **NVRTC**→**PTX**）编译。这才是 CubeCL 与「Rust 版 CUDA 源码」的本质区别。
...
**与 Burn 系列的关系**：[Burn 综合地图](blog-burn-summary.md) 讲编译期类型栈（`Autodiff<Fusion<…>>`）与运行时融合流（channel 架构 + 8.2×）；[ONNX AOT 编译器](blog-burn-onnx-summary.md) 讲构建期模型导入；本文从 CubeCL 内部写透 proc-macro、SSA 与 autotune，与前两篇互补。
...
| 手写 reduction 练 kernel | 并行读 [cubecl-book](cubecl/cubecl-book/src/SUMMARY.md) |

---

## 反直觉的心智模型：你写的不是 kernel，是编译过程
...
这就是为什么 CubeCL 不是"Rust 版的 CUDA"——你的 Rust 代码**不直接被编译为 GPU 指令**。你的 Rust 代码是一段**在 JIT 编译时运行的逻辑**，输出是 IR，IR 再被编译为 GPU 指令。你拥有这个编译过程。
...
CubeCL 的赌注是：**既然 expand 函数只是一个"向 Scope 里填指令"的过程，那用来填什么指令，可以在填的时候决定——不是写 kernel 时决定，是 JIT 编译时，根据目标平台的能力决定。**
...
**GPU 上永远不会执行这个 `if`。** `plane: true` 和 `plane: false` 会 JIT 出**两个不同的 kernel**——一个包含 `plane_sum` 指令，一个走标量求和路径。这避免了"在不支持 subgroup 的硬件上编译包含 subgroup 指令的 kernel"——而传统方案要么需要 `#ifdef`，要么会在 JIT 编译阶段报错（且错误信息不直观）。
...
`GemmBlueprint` 决定：dot product vs outer product 算法、stage 大小、是否 double-buffer、是否用 plane 级协作——每种组合对应**一份独立的 JIT 产物**。这就是为什么 CubeK 的 [`GUIDE.md`](https://github.com/tracel-ai/cubek/blob/main/GUIDE.md) 要求 blueprint 保持**极小**：防止 kernel explosion——comptime 参数每多一个取值，编译产物就翻一倍，磁盘缓存和编译时间可能失控。
...
CubeK matmul 的 tile 系统有 **13 种 `TileKind` 变体**（另有 `None` 作零初始化哨兵，不参与 autotune），定义于 cubek 仓库 `crates/cubek-std/src/tile/base.rs`：
...
CubeK（[tracel-ai/cubek](https://github.com/tracel-ai/cubek)，与 cubecl 分仓维护）在 CubeCL 上提供成品内核——matmul、attention、convolution、reduce、quant、FFT。它的核心不是算法本身（tiled matmul 谁都会写），而是 **Blueprint-Routine-Autotuner 的三层纪律**（[`GUIDE.md`](https://github.com/tracel-ai/cubek/blob/main/GUIDE.md)）：
...
这三层纪律的真实目的在 `GUIDE.md` 里写得很直白：**防止 kernel explosion。** 一个 3×3×3 的参数空间（3 种 algorithm × 3 种 stage size × 3 种 double-buffer 模式）= 27 份 JIT 产物。加上 13 种 TileKind autotune = 351 个候选。如果不严格控制哪些参数进 blueprint（编译维度）vs autotune（运行维度），组合爆炸会让 JIT 缓存大到不可接受。

---

## CubeCL 与 Burn 的边界

CubeCL **不是 Burn 的子项目**。它是一个独立的多平台 GPU 计算语言，可以被 Burn 消费，也可以独立使用。Burn 通过 `burn-cubecl` crate（及 `burn-cuda`、`burn-wgpu` 等后端包装）调用 CubeK 内核。
...
[Burn 综合地图](blog-burn-summary.md) 从 Burn 视角讲 fusion 调度与 8.2×；本篇从 CubeCL 内部展开 CFG → SSA 的优化 pass、Blueprint-Routine-Autotuner 纪律与 13 种 TileKind。
...
2. **冷启动有成本**。首次 launch 触发 JIT 编译 + autotune benchmark，用磁盘缓存和预填 autotune 数据可以缓解——但第一次永远是慢的。

与成熟 CUDA 生态的差距不在"能不能做"（同功能 matmul 已经做到），而在"做了多少年"：cuDNN 有三十年的算子优化积累和硬件代际适配。CubeCL 的算子库（CubeK）和 autotune 覆盖范围正在快速增长，但不可能在短时间内弥补这个历史差距。
...
| 术语 | 简要说明 |
|------|----------|
| **CubeCL** | Tracel 的多平台 GPU 计算框架：用 Rust + `#[cube]` 描述计算，JIT 生成 CUDA / HIP / WGSL / MLIR 等目标代码。 |
...
| **Fusion** | Burn 运行时把多算子合并调度（系列第三篇）；与 CubeCL JIT 是不同层。 |
...
*Burn 底层机制系列 · 第四篇 · [系列索引](blog-README.md)*
