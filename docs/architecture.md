# Tracel 生态的架构主线：Decision Deferral（决策推迟）

> Burn 把后端选择推迟到编译期，CubeCL 把 GPU 指令生成推迟到首次 launch，CubeK 把最优实现推迟到首次 benchmark——三个项目共享一条设计哲学，各解决不同层次的问题。

## 读前须知

- **本文性质**：跨项目分析——从 Burn、CubeCL、CubeK、Burn-ONNX 的源码中提取一条共用的模式：将关键决策从"写代码时"推迟到编译期、JIT 时或首次执行时。"decision deferral" 是本文使用的分析框架，不是 Tracel 项目代码中出现的术语。
...
> Burn、CubeCL、CubeK、Burn-ONNX 解决的是不同层次的问题，但从源码中可以提取一条共享模式：**将关键决策从"写代码时"推迟到编译期、JIT 时或首次执行时。** Burn 把后端选择推迟到编译期单态化；CubeCL 把 GPU 指令生成推迟到首次 launch；CubeK 把 tile 策略选择推迟到首次 benchmark；Burn-ONNX 把模型导入推迟到构建期 AOT 代码生成。"decision deferral" 是本文使用的分析框架，不是 Tracel 项目作者在源码中命名的概念。
