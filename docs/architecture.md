...
> Burn、CubeCL、CubeK 解决的是不同层次的问题。它们能像 `Fusion<CubeBackend<WgpuRuntime>>`（推理）或 `Autodiff<Fusion<CubeBackend<WgpuRuntime>>>`（训练）一样叠加，是因为每个项目通过 trait 定义了一层清晰的边界——上层只知道下层"能做什么"（trait 方法签名），不知道"怎么做"（具体实现）。

> **导读** · 难度：入门 · 预计 ~20 分钟 · [学习地图](../README.md#学习地图) 阶段 1
>
> - **读前应知道**：作为 Rust 工程师，trait / 泛型在这里基本无门槛；先过一页 [primer](primer.md) 补 NN 算子分类与训练步。唯一 Burn 专有点是装饰器后端嵌套。
> - **AI infra 通用映射**：这是"框架分层与后端抽象"问题，对应 PyTorch 的 Dispatcher 多级分发、XLA 的编译分层（基线见 [primer · Part B](primer.md#part-b--对比基线速查)）。
> - **本篇回答**：(1) 每个项目解决什么系统问题、在栈中处于哪一层；(2) 层与层通过哪些 trait 划定边界；(3) 为什么各层能独立演进
> - **配套练习**：无（概念篇，建立后续阅读的坐标系）
...
装饰器链（Device 类型在编译期单态化，例如默认 Wgpu = Fusion<CubeBackend<...>>；device.autodiff() 后外包 Autodiff）：
...
**问题**：PyTorch 把 autograd 嵌入 tensor 运行时——每个 tensor 携带 `grad_fn` 指针，推理时也有开销。怎么让 autodiff 可选？

**方案**：Autodiff 是装饰器，不是 tensor 的内置属性。`CubeBackend` 上没有 `.backward()`。`Autodiff<CubeBackend>` 上有。`78f10aec1` 起用户通过 `Device::wgpu(...).autodiff()` 把 device 路由到 `Autodiff<B>` 后端（运行时），叶子 tensor 再 `require_grad()` 才参与梯度图；cargo `autodiff` feature 控制是否链接 autodiff crate（编译期，推理二进制可整段排除）。
...
| 层 | 典型决策 | 时机 |
|----|----------|------|
| Burn 类型栈 | Device 对应哪种 Backend 组合（单态化） | `cargo build`（monomorphization） |
| Burn autodiff | 是否链接 autodiff crate / 运行时是否 `device.autodiff()` | cargo feature（编译期）/ `device.autodiff()`（运行时） |
...
下表是各子系统设计决策的跨框架对照——同一通用问题，三种分层选择。每行的"谁该用哪个"展开见对应系统设计文章的章末对比段。

| 维度 | 通用问题 | PyTorch | XLA | Tracel |
|------|---------|---------|-----|--------|
| **autograd 的位置** | autograd 放哪层、是否需梯度由什么定 | 嵌入 tensor（`grad_fn` 指针，运行时属性） | 嵌入 HLO 图（反向是图变换） | 装饰器（`Autodiff<B>`）+ `device.autodiff()` 运行时路由 + cargo feature 编译期可选 |
| **后端的切换** | 算子如何路由到具体后端 | 运行时 `tensor.to(device)` + Dispatch Key 查表 | 编译期（XLA 编译整个图） | `Device` 路由 + Backend 类型栈编译期单态化（用户侧无 Backend 泛型） |
| **算子融合** | 多 op 何时合成一 kernel | Dynamo trace + Inductor 编译 | XLA HLO fusion pass（编译期规则） | 惰性入队 + drain 时探索 + ExecutionPlanStore 缓存 |
| **GPU 代码生成** | kernel 源码如何变 GPU 二进制 | AOT（nvcc 预编译 CUDA kernel）+ Triton JIT | AOT（XLA → PTX/Metal） | JIT（首次 launch，`#[cube]` → IR → WGSL/SPIR-V/MSL） |
| **kernel 选择** | 为 shape/硬件选最快 kernel | 手写 CUDA kernel + Triton autotune（Python 参数网格） | 后端固定实现 | autotune（策略枚举 + 优先级剪枝 + anchor 缓存） |
| **模型导入** | 模型何时编译成可执行 | 运行时（ONNX Runtime / torch.onnx） | 运行时（TF Serving）+ AOT | AOT（`build.rs` 生成 Rust 源码） |
...
- Autodiff / Fusion / CubeCL / CubeK 各自解决哪一层的系统问题
- 层与层通过哪些 trait 划定边界，上层为什么无法绕过 trait 调用下层内部
- 哪些决策发生在编译期（Backend 单态化），哪些发生在运行时（Fusion drain、autotune benchmark）

> ✓ **完成自检**：能用自己的话解释"为什么 Burn 的 Autodiff 和 Fusion 可以独立演进而不会冲突"。
