...
> **导读** · 难度：难 · 预计 ~40 分钟 · [学习地图](../../README.md#学习地图) 阶段 6
>
> - **读前应知道**：JIT 篇的 `KernelId` 哈希与缓存 key；Autotune 篇的候选枚举
> - **AI infra 通用映射**：限制编译 key 维度以防 kernel 组合爆炸是通用问题，对比 CUTLASS 的模板参数爆炸（基线见 [primer · Part B](../primer.md#part-b--对比基线速查)）。
> - **本篇回答**：(1) 哪些参数进 JIT key、哪些留给 autotune；(2) Blueprint 离散化如何防止组合爆炸；(3) 与 CUTLASS 模板参数爆炸的差异
> - **配套练习**：无（CubeK 源码为可选 clone，文章已内联关键源码）
...
违反这条纪律的后果：如果 Blueprint 包含 `M`（问题大小的行数），每个不同的 `M` 值产生一个独立的编译产物——上千个 kernel。`BatchMatmulBlueprint`（`blueprint.rs:30`）遵守了这一纪律：它的字段全部是离散的结构性选择，不包含运行时变量。

### 第 2 层：Routine —— 硬件适配

```rust
// cubek/crates/cubek-matmul/src/routines/base.rs:18
pub trait Routine<RC: RuntimeConfig>: Sized {
    type Strategy: Default + Display + Clone;
    type Blueprint: Debug + Clone;
}

// base.rs:24 — 核心方法在 BatchMatmulRoutine 中
pub trait BatchMatmulRoutine<RC: RuntimeConfig>: Routine<RC, Blueprint: Blueprint> {
    fn launch<MA, R>(client, cube_dim, cube_count, ..., blueprint, ...) -> Result<(), MatmulSetupError>;
    fn expand_blueprint<R: Runtime>(problem, device_settings, strategy) -> Result<ExpandInfo<Self::Blueprint>, MatmulSetupError>;
    fn prepare<R: Runtime>(problem, device_settings, expand_info) -> Result<LaunchInfo<Self::Blueprint>, MatmulSetupError>;
    fn validate_blueprint<R: Runtime>(client, blueprint, ...) -> Result<(), MatmulSetupError>;
}
```

`Routine` 的职责是在 **JIT 编译 key 生成之前**，根据硬件和问题特征推断出最佳的 Blueprint。关键方法 `expand_blueprint` 做的是"探测 + 离散化"：

以 `SimpleAlgorithm` 为例（`routines/batch/simple.rs`，其 `expand_blueprint` 调用 `selector::infer_blueprint_plane`）：
...
```rust
// strategy/strategy.rs:559 — Strategy::Auto 的默认行为
fn auto<R: Runtime>(...) {
...
CubeK 的 autotune key（`strategy/tune_key.rs`）比 burn-cubecl 的 key 更丰富：
...
**通用问题**：一个 matmul kernel 要适配所有硬件和 shape，会产生大量变体——哪些参数进"编译 key"决定变体数量，变体太多则爆炸（编译产物或缓存膨胀）。任何模板化/JIT 化的算子库都要选：把参数空间限制在哪、爆炸落在哪一侧。
...
- **只针对 NVIDIA、要把变体固化进二进制** → CUTLASS：模板实例化在编译期完成，无运行时编译开销；代价是二进制体积随模板组合爆炸，且只服务 NVIDIA。
- **多平台、要线上 autotune、二进制要小** → CubeK：变体按 JIT key 哈希在运行时生成，二进制只含模板逻辑；代价是 JIT 缓存空间随 key 维度爆炸，需 Blueprint 纪律限制进 key 的参数。

两者爆炸的根因相同——把不该进 key 的参数放进了 key。CUTLASS 的"只允许某些模板参数组合"与 CubeK 的 Blueprint trait 边界 + Routine 离散化是同一纪律的两种执行方式。
...
2. 展开 Blueprint（Routine::expand_blueprint）
   ...
   → 输出 BatchMatmulBlueprint { tile_matmul: Cmma(16,16,16), swizzle: B32, ... }
...
4. **Autotune 候选数仍依赖 CubeCL 的优先级剪枝**：41 个 Strategy 变体 × 多种 hardware/problem 组合，全枚举仍然不小。CubeK 依赖 CubeCL 的 `TuneGroup` 优先级提前终止来控制数量。
...
- Strategy 枚举 + Auto 回退：`crates/cubek-matmul/src/strategy/strategy.rs`
- Autotune key：`crates/cubek-matmul/src/strategy/tune_key.rs`
...
- Blueprint（进 JIT key）、Routine（key 生成前离散化）、Autotuner（有限策略间选最快）三层各自的职责
- 如果把连续维度（如 M）放进 Blueprint，JIT 缓存会怎样爆炸
- CUTLASS 的模板参数爆炸是同一问题的另一种表现

> ✓ **完成自检**：能解释"如果把 M 放进 Blueprint，JIT 缓存会怎样爆炸"以及 CUTLASS 的等价问题是什么。
