...
对于 `MulScalar + AddScalar + Tanh`，三个 fuser 的 `ready == false`，只剩下 `ElementWiseFuser` 以唯一 ready 候选资格胜出。三个 fuser 根本没参赛，不存在"四个竞选掉三个"。这解释了 `BURN_FUSION_LOG=full` 日志中四条 `[fuser]` 都 `had 3 ops` 但最终 "fused ElementWise"——退赛的 fuser 也累积了操作数，但在 `find_best` 中被过滤掉了。
...
**通用问题**：把多个 op 合成一个 kernel 以省 launch 开销——决策放在哪一时机、需要什么前提、优化范围多大。任何带算子融合的框架都要选：编译期规则、运行时编译、还是惰性探索。
...
- **静态图、生产部署** → XLA：编译期规则零运行决策成本，但要静态图且规则覆盖有限。
- **手写 kernel 探索、单平台** → Triton：运行时编译灵活、优化空间大，但首次 autotune 秒级，需用户显式标注。
- **动态计算图 + 高重复性模式**（如 LLM 推理 decode，每次执行相同逐 token 序列）→ Burn：惰性探索不需静态图，首次 ~ms 探索后命中缓存；代价是计算图频繁变化时探索开销重复发生，且单 stream 连续 op 范围有限，跨 op 的大粒度融合不会发生。
- **已有 traced 图、想要编译期融合** → PyTorch Inductor：`torch.compile` 后用 FX graph cache，但要先 trace 成图，动态控制流靠 guard + 重编译。

一句话：XLA 把决策放编译期、Triton 放运行时第一次执行、Burn 放同步点但后续命中缓存——选择取决于"你的计算图静态吗、你愿意付多少首次开销、优化要多大粒度"。
