...
Triton 在运行时为 Python 函数生成优化的 GPU kernel。用户用 Python 描述计算，编译器做 tiling、向量化、autotuning。优点是灵活且优化空间大，缺点是编译开销高（首次运行时 autotune 可能耗时秒级），且需要用户显式标注可融合的计算。
...
Triton 在 Python 层面描述计算，`@triton.jit` 装饰器触发 JIT 编译（Python → Triton IR → Triton GPU IR → LLVM IR → PTX）。每次修改 kernel 逻辑都需要重新编译。Burn 的方案是 **comptime 泛型**——用 Rust 的编译期能力将操作序列编码为泛型参数，这样不同的操作序列对应不同的 monomorphized 函数实例，GPU 编译在 burn 本身的编译期发生，不是运行时。
