...
### 三篇分工（与 ONNX / CubeCL 专文对齐）

| 层次 | 文档 | 决策何时发生 | 本文覆盖 |
|------|------|-------------|---------|
| 构建期 | [ONNX 篇](blog-burn-onnx-summary.md) | `cargo build` | §七 入口 |
| 编译期 | **本文** §二–四 | `rustc` | 类型栈 |
| 运行期调度 | **本文** §五 | 训练/推理 loop | Fusion 流 |
| GPU 代码生成 | [CubeCL 篇](blog-cubecl-summary.md) | 首次 launch | §五 边界表 |
...
`Backend` trait 定义在 `crates/burn-backend/src/backend/base.rs`（`pub trait Backend:`，约 line 83）：
...
## 五、运行时：融合流与框架开销（v0.21.0 channel 重构）

> 本节展开 [第四节](#四第二层fusion四种张量全部包装) Fusion 层的运行时行为——操作如何入队、如何融合、以及为什么 0.21.0 的重构显著降低框架开销。
...
> **8.2× 脚注**：上表数字来自 Burn 团队在 **v0.21.0 前后** 的 fusion 引擎内部 benchmark（1024 reps、16 线程：733 ms → 89 ms），**未收录于本仓库公开 CI**；复现请以你 checkout 的 burn 版本为准。机制结论（递归锁 → `DeviceHandle` worker channel）见 `burn-fusion/src/client.rs` 的 `submit()` / `submit_blocking()` 与 `DeviceServiceStage::Upstream`。

### 融合流：推迟的是「算子怎么合并」

在 `Fusion<B>` 下，`tensor.matmul(&other)` 不会立刻触发 GPU matmul。它生成 `OperationIr::Matmul { lhs, rhs, out }`，进入当前流的队列。

`MultiStream`（`burn-fusion/src/stream/multi.rs`，`pub struct MultiStream`，约 line 102）：
...
**不读就不算。** 此外 Burn 还有 **增量融合**：`Processor` 把 op 喂给 Elementwise / Matmul / Reduce 等 fuser；fuser 返回 `Open` 或 `Closed`，关闭后写入 `ExecutionPlanStore`，新 fuser 继续吃下一段。同一条流上，前几段可能已执行，后几段仍在积累——**融合决策与执行流水线化**。channel 重构前，这条流水线被锁掐断；0.21.0 channel 重构后才恢复设计意图（见 §五脚注数据）。
...
| v0.21 的变化 | **channel 消除锁竞争** | 行为不变，但被锁拖累的上下游打通 |
| 只有一层时 | 能融合但可能选慢 kernel | 能选快 kernel 但多次 launch |

`#[cube]` 宏展开、`cubecl-opt` 的 SSA 管线、comptime 与 autotune 分工、13 种 `TileKind` 的 Blueprint 纪律——详见 [blog-cubecl-summary.md](blog-cubecl-summary.md)（含 **Vectorization / Comptime / Autotune 三列表**）。
...
## 七、ONNX 入口：构建期 AOT，而非运行时加载

Burn 的 ONNX 支持不是运行时「加载 `.onnx` 再解释执行」——而是在 `build.rs` 里运行的 **AOT 编译器**，把 ONNX **翻译**为可调试的 Rust 源码与 `model.bpk` 权重。**源 `model.onnx` 文件仍在磁盘**；变化是**运行时二进制不再依赖** ONNX Runtime、protobuf 解析或图解释器。

典型用法：`ModelGen::new().input("model.onnx").out_dir("model/").run_from_script()` → 得到 `model.rs` + `model.bpk` → `Model::from_file(…)` + `forward()`。

**完整对比**（ORT vs Burn ONNX）、**6 阶段 IR 流水线**、**注意力融合 pass**、**SDXL 分区**、**三层测试与 `expectations.toml`**——见专文 [blog-burn-onnx-summary.md](blog-burn-onnx-summary.md)（含可复验统计脚本）。

生成的 `model.rs` 是普通 Burn 代码——穿过本文的 `Autodiff<Fusion<CubeBackend>>` 栈，走融合流，并在首次遇具体形状时触发 [CubeCL 的 JIT 与 autotune](blog-cubecl-summary.md)。
...
0.20.1：16 线程整条路径约 **733 ms**。0.21.0：约 **89 ms**（见 §五脚注）。瓶颈在 channel 之前的锁，不在 matmul 的 PTX。
...
| **8.2×** | v0.21.0 channel 重构后，团队内部 benchmark 约 8.2×（733 ms → 89 ms，16 线程）。根因是锁消除后融合与执行可流水线并行；见 §五脚注。 |
| **Drain** | 排空融合流：`read_float` 等操作触发 `drain_stream`，将队列中所有 op 融合并提交执行。不读就不算。 |
| **BackendRouter** | `burn-router` 的多后端路由 decorator；运行时把不同 op 转发到不同底层 backend。 |
...
| **注意力融合** | `coalesce_attention.rs`（约 1367 行）：识别 SDPA 分解模式，融合为单一 `Attention` 节点。 |
| **分区编译** | SDXL 级大图切成 64–256 节点的子模块（`partition.rs`）。 |
| **IR 流水线** | Protobuf → 类型推断 → 8 个 simplify pass × 定点迭代 → finalization → codegen。详见 [ONNX 篇](blog-burn-onnx-summary.md)。 |
...
*Burn 底层机制系列 · 综合地图 · [ONNX 篇](blog-burn-onnx-summary.md) · [CubeCL 篇](blog-cubecl-summary.md) · [系列索引](README.md)*
