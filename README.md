...
需要了解：
- **Rust trait/generics**：Burn 的类型栈（`Autodiff<Fusion<CubeBackend>>`）通过 trait 嵌套实现
- **GPU 基础概念**：kernel launch overhead、workgroup、tile、寄存器/shared-memory/global-memory 三级存储层次
- **基本的 ML 知识**：什么是 forward/backward pass、矩阵乘法、激活函数

不需要了解：
- CubeCL 或 wgpu 的具体 API——文章会从零展开
- 任何 Rust proc-macro 的知识——JIT 文章会逐步解释 `#[cube]`
